...
inline std::string device_prop_string(cudaDeviceProp prop) {
  ...
  double memBandwidth = (memClockRate * 1000.0) *
    (prop.memoryBusWidth / 4.0) / 1.0e9;
  ...
  std::string s = detail::stringprintf(
    ...
    memClockRate / 1000.0, prop.memoryBusWidth, memBandwidth,
...
public:
  ...
  mem_t(memory_space_t s = memory_space_device)
      : _context(nullptr), _pointer(nullptr), _size(0), _space(s) {}
